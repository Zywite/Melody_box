...
def test_create_user(db):
    user = UserService.create_user(db, username="newuser", email="new@example.com", password="pass123")
...
def test_verify_user_password_correct(db, test_user):
    user = UserService.verify_user_password(db, "test@example.com", "password123")
    assert user is not None
    assert user.id == test_user.id


def test_verify_user_password_wrong(db, test_user):
    user = UserService.verify_user_password(db, "test@example.com", "wrongpassword")
    assert user is None


def test_verify_user_password_email_not_found(db):
    user = UserService.verify_user_password(db, "no@example.com", "password123")
    assert user is None


def test_create_user_unique_email(db):
    UserService.create_user(db, username="user1", email="same@example.com", password="pass1")
    with pytest.raises(Exception):
        UserService.create_user(db, username="user2", email="same@example.com", password="pass2")


def test_create_user_unique_username(db):
    UserService.create_user(db, username="sameuser", email="user1@example.com", password="pass1")
    with pytest.raises(Exception):
        UserService.create_user(db, username="sameuser", email="user2@example.com", password="pass2")
