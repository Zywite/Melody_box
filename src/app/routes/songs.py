...
async def _process_upload_file(
    ...
    if file_ext not in allowed_exts:
        raise HTTPException(
            status_code=400,
            detail=ERROR_INVALID_FILE_FORMAT.format(
                filename=file.filename,
                extensions=', '.join(allowed_exts)
            )
        )
    ...
    try:
        ...
        if os.path.exists(file_path):
            os.remove(file_path)
        raise HTTPException(status_code=500, detail=ERROR_UPLOAD_PROCESSING.format(filename=file.filename, error=str(e)))
...
def stream_song(song_id: str, db: Session = Depends(get_db)):
    ...
    return FileResponse(file_path, media_type=media_type, headers={
        "Accept-Ranges": "bytes",
        "Cache-Control": f"public, max-age={STREAM_CACHE_MAX_AGE_SECONDS}"
    })
...
async def upload_song(
    ...
    result["message"] = SUCCESS_UPLOAD_MESSAGE + (f" (FFT en cola, tarea: {result['fft_task_id']})" if result['fft_task_id'] else "")
    return result
...
async def get_song_fft(request: Request, song_id: str, db: Session = Depends(get_db)):
    ...
    if job_id:
        ...
        try:
            ...
            task.error = str(e)
            db.commit()
            raise HTTPException(status_code=500, detail=ERROR_FFT_ANALYSIS_FAILED.format(error=str(e)))
